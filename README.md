...
**四级权限体系**：
- 🔴 **Admin (系统管理员)** - 拥有最高权限，可进行全系统配置、用户管理、数据库维护。
- 🟠 **Engineer (工程师)** - 负责设备参数调试、维护管理，可访问用户管理，但不可修改系统核心配置。
- 🟡 **Operator (操作员)** - 负责日常监控与门控操作，无配置修改权限。
- 🔵 **Observer (观察员)** - 仅拥有只读查看权限，禁止退出系统（适用于无人值守大屏展示）。

**安全机制**：
- **自动降级** - 非观察员用户登录 2 小时后，系统自动注销并降级为观察员模式，防止权限滥用。
- **MD5 加密** - 所有用户密码采用 MD5 加密存储，登录时支持自动升级旧版明文密码。
- **防退出保护** - 观察员模式下禁用退出功能，强制保持监控运行；退出系统需强制密码校验。
